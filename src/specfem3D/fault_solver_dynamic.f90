...
module fault_solver_dynamic
  ...
  real(kind=CUSTOM_REAL), save :: V_HEALING        !slip velocity threshold for healing (not very robust) 
  real(kind=CUSTOM_REAL), save :: V_RUPT           !slip velocity threshold for rupture front
  integer, save                :: NTOUT,NSNAP      !Number of time steps: NTOUT
  integer, save                :: Nfaults          !Number of faults
...
!=====================================================================
!=====================================================================
! Initialisation of dynamic fault
!
! It contains subtoutines: BC_DYNFLT_init,       init_one_fault
!                          init_2d_distribution, init_fault_traction
! 
! BC_DYNFLT_init   (public subroutine)
!     input: 
...
! init_one_fault
!     input: 
!            bc, IIN_BIN, IIN_PAR, dt, NT, iflt, myrank
!
...
subroutine init_one_fault(bc,IIN_BIN,IIN_PAR,dt,NT,iflt,myrank)
  ...
  call init_dataXZ(bc%dataXZ,bc) ! output a fault snapshot at t=0
  ...
  contains
    ...
    real(kind=CUSTOM_REAL), dimension(bc%nglob) :: loc_str,loc_dip,sigma0,tau0_str,tau0_dip,&
                                    Rstress_str,Rstress_dip,static_fc,dyn_fc,swcd,cohes,tim_forcedRup
    ...
    integer                      :: relz_num,sub_relz_num, num_cell_str,num_cell_dip 
...
!=====================================================================
!=====================================================================
! Solve friction on dynamic faults
!
! It contains subtoutines: bc_dynflt_set3d_all, BC_DYNFLT_set3d_one_fault
! 
! bc_dynflt_set3d_all   (public subroutine)
!     input: 
!            F, V, D
...
subroutine bc_dynflt_set3d_all(F,V,D)
  ...
  do i=1,size(faults)
     call BC_DYNFLT_set3d_one_fault(faults(i),F,V,D,i)
  enddo
...
!---------------------------------------------------------------------
  ...
  subroutine BC_DYNFLT_set3d_one_fault(bc,MxA,V,D,iflt)
...
end subroutine BC_DYNFLT_set3d_one_fault
...
!
! It contains subtoutines: swf_init, swf_update
! and function:            swf_mu
! 
! swf_init
!     input: 
...
!
! It contains subtoutines: rsf_init, rsf_update_state, funcd, rtsafe
! and function:            rsf_mu (will consider to add)
! 
! rsf_init
!     input: 
!            f, T0, V, nucFload, coord, IIN_PAR
!
! rsf_update_state
!     input: 
!            V, dt, f
...
! funcd
!     input: 
!            x, fn, df, tStick, Seff, Z, f0, V0,
!            a, b, L, theta, statelaw
!
! rtsafe
!     input: 
!            x1, x2, xacc, tStick, Seff, Z, f0, 
!            V0, a, b, L, theta, statelaw
!
!=====================================================================
     ...
     if (f%stateLaw == 1) then
        f%theta = f%L/f%V0 &
                  * exp( ( f%a * log(TWO*sinh(-sqrt(T0(1,:)**2+T0(2,:)**2)/T0(3,:)/f%a)) &
                  - f%f0 - f%a*log(f%V_init/f%V0) ) / f%b )
     else
...
!
!   store_dataXZ
!      input: dataXZ, stg, dold, dnew, dc, vold, vnew, timeval, dt
!
!   write_dataXZ
...
!=====================================================================
!   Predefined mathematical functions listed:
!
!   asinh_slatec
!      input x (double); output asinh function
!
!   heaviside
!      input x (doble); output heaviside function
!=====================================================================
...
!=====================================================================
!   GPU subroutines listed:
!
!   transfer_faultdata_GPU
!
!   rsf_GPU_init
!
!   synchronize_GPU
